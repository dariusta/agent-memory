...
- **GPU workers — RunPod serverless** ([[runpod]]). The ML back-ends under `services/` (`voice-isolation`, `comfy-realism`, `qc-inference`, `phone-relay`) each run as a RunPod endpoint invoked from Trigger jobs. The **voice-isolation** worker cleans a scraped UGC clip into a VoxCPM reference; its output-cleaning path + the deliberate noise-vs-authenticity tradeoff live in [[voice-isolation-pipeline]] (the yield/gate half is [[voice-scrape-isolation-pipeline]]). Deploy foot-gun — env flip is live but a code change needs an image rebuild: [[runpod-serverless-env-vs-image]].
